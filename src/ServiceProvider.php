...
class ServiceProvider extends BaseServiceProvider
{
    ...
    {
        if (File::isDirectory($views = $this->modulePath('resources/views'))) {
    ...
    {
        if (File::isDirectory($langs = $this->modulePath('lang'))) {
    ...
    {
        if (File::isFile($config = $this->modulePath('config.php'))) {
            $this->mergeConfigFrom($config, $this->slug);
        }

        if (File::isDirectory($path = $this->modulePath('config'))) {
    ...
    {
        if (File::exists($this->modulePath('app/Console/Kernel.php'))) {
    ...
    {
        return modulePath($this->module, $path);
    }
}
